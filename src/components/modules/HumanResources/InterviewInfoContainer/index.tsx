...
function InterviewInfoModule() {
  ...
  useEffect(() => {
    if (result?.entranceInterview?.status === "NEW") {
      setCurrent(0);
    }
    if (result?.entranceInterview?.status === "SENT_INTERVIEW") {
      setCurrent(2);
    }
    if (
      result?.entranceInterview?.status !== "SENT_INTERVIEW" ||
      result?.entranceInterview?.status !== "NEW"
    ) {
      setCurrent(3);
    }
  }, [result]);
