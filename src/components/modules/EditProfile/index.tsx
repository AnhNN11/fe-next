...
import { Checkbox, Col, Flex, Form, FormProps, Input, Row, Select } from "antd";
...
function EditProfileModule() {
  ...
  const { t } = useTranslation(params?.locale as string, "editProfile");
  ...
  const onLoginGG = async () => {
    try {
      ...
      await signUp(result).unwrap();
      console.log(signUp(result).unwrap());
      router?.push(`/${locale}/sign-in`);
    } catch (error) {}
  };

  const { firstname, lastname } = detachName(userInfo?.decodedToken?.name);
