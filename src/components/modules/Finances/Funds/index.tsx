...
import { useGetFundMutation, useGetFundTokenMutation, usePayFundMutation } from "@/store/queries/finances";
...
function FinanceFundsModule() {
	...
	const ExpandRowRender = (row: any) => {
		...
		return (
			<Table columns={columns} dataSource={data[row._id] ?? []} pagination={false} rowKey={(record) => record._id} />
		);
	};
